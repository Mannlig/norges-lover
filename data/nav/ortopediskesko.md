<!-- innholds-hash: 95afe30a905cfdb085f5015ee3524305d535e3f2f4ec0887befc426e94ca4bb4 -->

# Ortopediske sko - nav.no

## Kildeinformasjon

- **Kilde:** NAV (Arbeids- og velferdsdirektoratet) – https://www.nav.no/ortopediskesko
- **Sist hentet:** 2026-06-27T00:29:03Z

## Innhold

Hjelpemiddel og Pengestøtte


## Ortopediske sko

Hjelper deg når du har ulike problemer med føttene.


### Innhold på denne siden

Det finnes også informasjon om ortopediske sko til


### Innhold på denne siden

Det finnes også informasjon om ortopediske sko til

Du kan få ortopediske sko hvis du har nedsatt funksjonsevne i mer enn to år, og

- har en feilstilling i foten eller ankelen
- har problemer med føttene på grunn av diabetes eller revmatisme
- har en annen funksjonsnedsettelse i føttene
Du må være

Du kan få støtte til:

- Fabrikkprodusert ortopediske sko som er forhåndsgodkjent av Nav.
- Tilpassing av sko som du har kjøpt selv.
- Ortopediske sko som er laget etter mål.

#### Antall skopar

Første gang du får støtte til sko, kan du få inntil fire par. Senere kan du få støtte inntil to par sko per kalenderår.

Barn i vekst og personer som har en funksjonsforstyrrelse som gjør at de sliter mer på skoene enn normalt, kan få støtte til mer enn to par per kalenderår.


#### Forskjellig størrelse på føttene

Hvis du må kjøpe to identiske par sko i forskjellig størrelse for å få ett fungerende par, kan du få dekket merutgiftene. Du må ha minst to hele skonummer (1,5 cm) forskjell på føttene.

Du kan få dekket inntil to ekstra par sko hvert kalenderår med inntil 875 kroner per par. Du må selv legge ut og søke om tilbakebetaling.

Ser du etter informasjon om fotsenger (spesiallagde innleggssåler), finner du dette på siden for ortoser.

Du må selv betale for en del av de ortopediske skoene. Dette betaler du direkte til leverandøren.

- Egenbetalingen for spesialsydde ortopediske sko til personer over 10 år er 741 kroner per par.
- Egenbetalingen for spesialsydde ortopediske sko til barn under 10 år er 450 kroner per par.
- Støtte til fabrikkproduserte ortopediske sko er 1 800 kroner per par. Du må betale det overskytende selv.
Egenbetalingen inngår ikke i grunnlaget for frikort.


#### Yrkesskade

Trenger du ortopediske sko på grunn av en godkjent yrkesskade? Da vil du få refundert egenbetalingen ved å søke om refusjon første gangen du kjøper ortopediske sko. Etter dette er det egenbetaling.

Nav dekker dine nødvendige reiseutgifter til ortopeditekniske verksteder innenfor helseregionen din. Du får dekket:

- Billigste reisemåte med offentlig transport
- Taxi eller lignende hvis helsetilstanden din krever det
- Kilometergodtgjørelse ved bruk av privat bil.
Første gang du trenger ortopediske sko skal en sykehuslege eller legespesialist fylle ut søknaden og sende den til et ortopedisk verksted som har rammeavtale med Nav. Det ortopediske verkstedet sender inn søknaden til Nav.

Neste gang du har behov for ortopediske sko, fyller det ortopediske verkstedet ut søknaden og sender den til Nav.

Flere av leverandørene har i tillegg underavdelinger i alle landsdeler. Nav dekker nødvendige reiseutgifter til ortopediske verksteder innenfor helseregionen din. Du må selv undersøke hvilke leverandører som har underavdeling i din helseregion.


#### Atterås AS

Møllendalsveien 1, 5009 Bergen, tlf.: 93 68 60 00, e-post:


#### Bahr Ortopedi AS

Fornebuveien 42, 1366 Lysaker, telefon 919 24 818. e-post:


#### Barnetekniske AS

Ingvald Ludvigsens gate 20, 3027 Drammen, tlf.: 32 82 3018, e-post:


#### Blatchford Ortopedi AS

Skibåsen 42 B, 4636 Kristiansand, tlf: 38 02 32 00, e-post:


#### BT Ortopedi AS

Ingeniør Rybergs gate 114, 3027 Drammen, tlf. 32 80 93 93, e-post:


#### Drevelin AS

Møllendalsveien 8, 5009 Bergen, tlf.: 55 20 64 64, e-post:


#### Drevelin Ortopedi Sør AS

Kongsgård Allé 53, 4632 Kristiansand, tlf.: 40 00 48 08, e-post:

Innspurten 9, 0663 Oslo, tlf.: 23 28 82 00, e-post:


#### Hamar Ortopediske AS

Torggata 1, 2317 Hamar, tlf.: 62 51 19 50, e-post:


#### Moss Ortopediske Klinikk AS

Prinsens gate 9, 1530 Moss, tlf.: 69 25 00 35, e-post:


#### Norsk Ortopedisk Fottøy AS

Oslogata 4, 1610 Fredrikstad, tlf: 69 30 00 10, e-post:


#### Norsk Teknisk Ortopedi AS

Vikavegen 17, 2312 Ottestad, tlf.: 62 57 44 44, e-post:


#### Ortopediteknikk A/S

Ryensvingen 6, 0680 Oslo, tlf.: 23 03 56 00, e-post:


#### Ortoteket AS

Bussedalen 3, 3616 Kongsberg, tlf.: 48 43 61 00, e-post:


#### Sophies Minde Ortopedi AS

Brynsveien 14, 0667 Oslo, tlf.: 22 04 53 60, e-post:


#### Teknomed AS

Skolmar 13, 3232 Sandefjord, tlf.: 33 45 45 33, e-post:


#### Trøndelag Ortopediske Verksted AS

Professor Brochs gate 8C, 7030 Trondheim, tlf.: 73 84 56 30, e-post:


#### ØSTO Ortopedisenter AS

Gartnerveien 10, 2312 Ottestad, tlf.: 62 57 39 00, e-post:


#### Trenger du nye ortopediske sko?

Kontakt det ortopediske verkstedet når du skal søke om nye ortopediske sko.


#### Søknad om stønad til alminnelig fottøy ved ulik fotstørrelse


#### Søknad om refusjon av betalt egenandel for fottøy eller fotseng ved yrkesskade

Du får skriftlig vedtak når søknaden er behandlet.


#### Saksbehandlingstid for søknader

Saksbehandlingstiden er tiden fra vi får søknaden din og til vi har gjort et vedtak. Husk at vi trenger all nødvendig dokumentasjon for å behandle søknaden din.

For mer informasjon om leveringstid, ta kontakt med det ortopediske verkstedet.

Når du skal ha et ortopedisk hjelpemiddel, kan det hende du trenger å møte opp for prøving og tilpassing flere ganger.

Hvis du trenger å justere, korrigere eller reparere hjelpemiddelet, tar du kontakt med det ortopediske verkstedet.

Har du fått et vedtak fra oss som du mener er feil? Da kan du klage til Nav-enheten som skrev vedtaket. De vil vurdere saken din på nytt. Hvis de ikke er enig i klagen din, sender de den videre til Nav klageinstans.


#### Klage på vedtak

I vedtaket står det hvordan du går fram hvis du skal klage, hvem du skal klage til og klagefrist. Hvis du har spørsmål om vedtaket, kan du


#### Anke vedtak

Hvis du er uenig i svaret på klagen din fra Nav klageinstans, kan du med noen unntak anke vedtaket. Fristen for å anke står i vedtaket.


#### Saksbehandlingstid for klage og anke

Har du fått et vedtak fra oss som du mener er feil? Da kan du klage til Nav-enheten som skrev vedtaket. De vil vurdere saken din på nytt. Hvis de ikke er enig i klagen din, sender de den videre til Nav klageinstans.

Oppdatert 05.12.2025


#### Skriv til oss

Still oss et spørsmål og få skriftlig svar.


#### Ring oss på 55 55 33 33

Stengt nå, åpner i morgen kl. 9


#### Chat med oss

Du møter først chatbot Frida. Du kan be Frida om å få chatte med en veileder (hverdager 09.00–15.00).

Alltid åpen

Stengt nå, åpner i morgen kl. 9


---
*Automatisk hentet fra [NAV](https://www.nav.no/ortopediskesko) av norges-lover-bot.*

## Endringshistorikk

- **2026-05-18** Første gang hentet
- **2026-05-23** Innhold endret (se git-historikk for diff)
- **2026-06-03** Innhold endret (se git-historikk for diff)
- **2026-06-11** Innhold endret (se git-historikk for diff)
- **2026-06-19** Innhold endret (se git-historikk for diff)
- **2026-06-27** Innhold endret (se git-historikk for diff)
