<!-- innholds-hash: 6c860e91d112eb82b7632610d14daff26a0a622492011a1e29fad9787a1335f3 -->

# Søke, bestille og bytte hjelpemidler - nav.no

## Kildeinformasjon

- **Kilde:** NAV (Arbeids- og velferdsdirektoratet) – https://www.nav.no/bestillingsordningen
- **Sist hentet:** 2026-06-19T20:45:15Z

## Innhold

For hjelpemiddelformidlere og andre samarbeidspartnere


## Søke, bestille og bytte hjelpemidler

Om digitale og papirbaserte løsninger for fagpersoner som har ulike roller i hjelpemiddelformidlingen.


### Innhold på denne siden

Det finnes også informasjon om søke, bestille og bytte hjelpemidler til


### Innhold på denne siden

Det finnes også informasjon om søke, bestille og bytte hjelpemidler til

Hvis du er fagperson i kommunen og skal søke hjelpemidler på vegne av en innbygger, kan du melde behov om hjelpemidler digitalt.

Foreløpig gjelder den digitale behovsmeldingen kun et utvalg av hjelpemidler innen utvalgte kategorier, for både voksne og barn. For andre typer hjelpemidler må du fortsatt søke på papir.

Du finner alle skjema for fagpersoner, både digitale og papirbaserte, i


#### Ulike tilganger til digital behovsmeldig

Det er to ulike tilganger til den digitale tjenesten: full tilgang og begrenset tilgang. Hvilken tilgang du skal ha kommer an på din rolle i kommunen, din erfaring med hjelpemidler og tilrettelegging og avtale med leder i egen kommune.

Tilgangen kan gis til kvalifiserte fagpersoner som kjenner regelverket og ansvarsfordelingen på hjelpemiddelområdet, og som har god faglig kunnskap om hjelpemidler og tilrettelegging.

For å få denne tilgangen må du ha fått tildelt en Altinn-rettighet av din arbeidsgiver.

Med full tilgang kan du søke på

Tilgang kan gis til personer som har tatt den obligatoriske e-læringen "Godkjenningskurs bestillingsordningen". Du må i tillegg ha avtale med leder om at du kan be om hjelpemidler fra Nav.

Med denne tilgangen har du mulighet til å bestille hjelpemidler og tilbehør til innbygger i kommunen du jobber i fra


#### Hvordan få tilgang

Første gang din kommune skal ta i bruk digital søknad er det

For å få full tilgang til våre digitale tjenester, kan du be om dette via lenken i digital behovsmelding. Tilgangsstyrer i din kommune vil få en melding og  kan gi deg nødvendige rettigheter i Altinn.

Dersom du skal ha begrenset tilgang, skal det ikke gjøres noe i Altinn. Da kan du som ansatt i kommunen ordne nødvendig tilgang selv. For å få denne tilgangen må du:

- være ansatt i en kommune. Nav sjekker Arbeidstaker- og arbeidsgiverregisteret (Aa-registeret)
- ha bestått
Første gang du sender inn en digital bestilling, må du krysse av for at din leder ønsker at du skal kunne be om hjelpemidler fra Nav.


#### Når må man bruke papirskjema?

- Fagpersoner som ikke jobber i kommunen har ikke tilgang til digital behovsmelding og må søke på papir når de skal søke om hjelpemidler på vegne av andre
- Du må bruke papirskjema hvis du skal søke om hjelpemidler på vegne av innbygger som har adresseskjerming i Folkeregisteret.
Du finner skjema i

Du må alltid begrunne at hjelpemiddelet er nødvendig og hensiktsmessig.

For en del hjelpemidler krever Nav at det legges ved et utfylt tilleggsskjema (T-skjema). Dette brukes for at vi skal få de opplysningene vi trenger for å kunne behandle søknaden, og for å finne eller tilpasse riktig hjelpemiddel.

Tilleggsskjemaene finner du kun på skjemaoversikten for samarbeidspartnere. Du velger først hva slags hjelpemiddel du skal søke (hørsel, bevegelse, kommunikasjon osv.). De aktuelle skjemaene for hjelpemiddeltypen dukker opp etter hvert som du gjør valg innover i løsningen. For noen hjelpemiddeltyper finnes det flere T-skjema, for andre er det bare ett.

Det er ikke alltid åpenbart om det trengs tilleggsskjema når du skal søke om et hjelpemiddel. Typiske eksempler på at det er nødvendig med tilleggsskjema er når det er behov for eksakte mål, eller om bruker har tilleggsvansker som er relevante for bruken hjelpemiddelet. Hvis du finner den aktuelle hjelpemiddeltypen på

Tilleggsskjema gjelder bare når du søker på papir.

Når du søker om hjelpemidler via digital behovsmelding og har full tilgang, kan du også søke om tilbehør. Du kan melde behov for tilbehør

- samtidig med søknaden om hjelpemiddelet
- til et hjelpemiddel bruker allerede har i utlån
- til et hjelpemiddel det ennå ikke er fattet vedtak på.
Noen typer tilbehør regnes som standard tilbehør, mens du må begrunne behovet for noen typer tilbehør.

Det er ikke nødvendig med fullmakt når du melder behov om tilbehør.


#### Søke om tilbehør på papir

Hvis du ikke har full tilgang på digital behovsmelding, må du søke om tilbehør på papir.

Benytt samme søknadsskjema som for hovedhjelpemiddelet når du søker om tilbehøret.

Det er mulig å søke om piggdekk til scooter og elektrisk rullestol. Når du søker om piggdekk må du begrunne behovet. Piggdekkene fører med seg noen ulemper, for eksempel kan man ikke bruke stolen inne på et handlesenter. Piggdekk gir også større fare for punktering. Du må sammen med bruker veie opp behovet for piggdekk mot ulempene det fører med seg. Som alternativ til piggdekk kan du forsøke å slippe ut litt luft i dekkene for å øke kontaktflaten.

Ved behov kan du søke om hoftebelte, vest og andre belter for posisjonering i for eksempel rullestoler, arbeidsstoler, sykler eller vogner. Hvis det er behov for belte eller sele for å hindre personen i å gå ut av stolen, må det foreligge

Når det er behov for særlig rask utlevering av hjelpemiddel, skal det meldes inn i digital behovsmelding under steg 5: "Levering". Det er kun ansatte i kommunen som kan gjøre dette.

Ansatte ved sykehus og privateide institusjoner kan ikke bruke digital behovsmelding og må kontakte kommunen som brukeren bor i.

Hjelpemiddelsentralen vurderer i hvert tilfelle om det er en hastesak. Hastesaker vil få høyest prioritet i vår saksbehandling. Du kan følge med på status i saken på Dine hjelpemiddelsaker.

Årsaken til at det haster skal være begrunnet i innbyggers sykdomstilstand, livssituasjon og i sammenhengen hjelpemidlet skal brukes i. Hjelpemiddelsentralen behandler saken som hastesak når

- bruker trenger hjelpemiddel for å hindre videre utvikling av trykksår, eller fordi det er stor fare for utvikling av trykksår
- bruker må ha terminalpleie
- bruker skrives ut fra sykehus og det var ikke mulig å planlegge i forkant
- det har skjedd en rask forverring av en alvorlig diagnose
Når det er behov for enkle hjelpemidler som ikke krever individuelle tilpasninger kan du bestille disse via bestillingsordningen. Forskjellen på en bestilling og en søknad er først og fremst at en bestilling kvalifiserer til forenklet saksbehandling hos Nav. Kommunale ansatte med tilgang til digital behovsmelding bruker alltid digital behovsmelding, også til denne typen hjelpemidler.

Du må være godkjent bestiller for å bestille hjelpemidler. For å bli godkjent bestiller, må du ta

Personen du skal bestille til, må ha et vedtak på hjelpemiddel fra Nav fra før.

Dersom du er kommunalt ansatt, er godkjent bestiller men ikke har tilgang på digital behovsmelding, kan du benytte digital bestillingsordning.

Dersom du er godkjent bestiller men ikke er kommunalt ansatt, må du bestille på papir.

Bruk knappen "Send bestilling" uansett. Deretter velger du "Send digitalt" hvis du skal bestille digitalt. Hvis du skal bestille på papir, velger du "Send i posten".

Når vi mottar bestillingen sjekker vi at innsender er godkjent bestiller og får det inn i vårt lager- og logistikksystem.

Dersom du har bestilt digitalt, kan innbygger følge fremdriften i egen sak ved å logge inn på nav.no. Dette er ikke mulig ved papirbestilling.

Vi kan ta stikkprøver for å se om bestillingen er i tråd med regelverket.

Disse hjelpemidlene inngår i ordningen og kan bestilles. For andre hjelpemidler må du sende ordinær søknad.

Du finner også en

10.02.2026 Rettet trykkfeil

Hvis du har sendt en søknad til Nav hjelpemidler og tilrettelegging, kan du logge inn på nav.no og sjekke saken din selv.

Innloggingsknappen finner du øverst til høyre på siden.

Hvis du har brukt digital behovsmelding til å søke hjelpemidler for en innbygger, kan du sjekke

- status i sak for de hjelpemiddelsøknadene du selv har søkt om på innbyggers vegne.
Søknader på papir er det bare innbygger selv som har tilgang til gjennom nav.no.

Hvis privatpersoner logger inn selv, kan de sjekke

- status i sine pågående saker
- oversikt over utlånte hjelpemidler (utlånsoversikt)
- oversikt over innsendte saker, dokumenter og vedtak.
Foreldre har ikke tilgang til å følge barnas sak på nav.no. De kan ringe til Nav hjelpemiddelsentral hvis de trenger informasjon om status på saken til eget barn.

Innbygger får melding når Nav har behandlet søknaden og hjelpemiddelet er klart til å sendes til kommunen. Dette gjelder ikke hvis vedkommende har reservert seg mot digital kommunikasjon fra Nav.

- Av og til må man bytte hjelpemiddel, for eksempel hvis det er utslitt eller det ikke passer lenger. Bruker kan ta kontakt med kommunen for å få hjelp til å bytte.
- Du kan bare bytte til samme type hjelpemiddel. Hvis behovet har endret seg slik at brukeren trenger et annet hjelpemiddel, må det søkes på nytt.
- Du kan også bytte dersom hjelpemiddelet er mistet eller ødelagt.
- Du kan ikke bytte fastmonterte hjelpemidler, briller eller aktivitetshjelpemidler for personer over 26 år. Disse hjelpemidlene krever ny søknad hvis det er behov for noe nytt.

#### Bytte hjelpemidler med tilgang til digital behovsmelding

Ansatte i kommunen med full tilgang kan bytte hjelpemidler via digital behovsmelding. Når du skal bytte, velger du først hjelpemiddelet du skal bytte til, deretter melder du inn hjelpemiddelet du skal levere tilbake.

Det er kun mulig å melde bytte digitalt om hjelpemidler som er tilgjengelig via digital behovsmelding. Hjelpemidler som ikke er tilgjengelig ennå må byttes via bytteskjema.


#### Bytte hjelpemidler uten tilgang til digital behovsmelding

Har du ikke full tilgang kan du benytte bytteskjema.

Du trenger ikke å innhente fullmakt når du skal bytte et hjelpemiddel på vegne av en bruker.

Våre digitale tjenester har et høyt sikkerhetsnivå for å ivareta personvern til innbyggerne og beskytte sensitive helseopplysninger. Hver gang du skal logge inn i våre digitale tjenester må du logge inn med ID-porten.

At du er logget inn, betyr bare at du er identifisert. Ingen private opplysninger om deg brukes i søknaden. Informasjon som du fyller ut etter at du har logget inn på sikker måte knyttes til hjelpemiddelbrukeren du søker på vegne av.

Om du ønsker å lese mer om hjelpemidler til bestemte situasjoner eller funksjonsvariasjoner, kan du lese på

For deg som jobber med hjelpemiddelformidling anbefaler vi at du leser

På Kunnskapsbanken vil du også finne blant annet fagartikler, utdypende informasjon om tilrettelegging og kartlegging, dybdeinformasjon om en del hjelpemiddeltyper og flere godkjenningskurs.

Oppdatert 08.05.2026


#### Ring oss på 55 55 11 11

Stengt nå, åpner mandag kl. 9


#### Nyhetsbrev

Meld deg på nyhetsbrevet fra Nav hjelpemidler og tilrettelegging for å få oppdateringer om regelverk, tjenester og kurs.


---
*Automatisk hentet fra [NAV](https://www.nav.no/bestillingsordningen) av norges-lover-bot.*

## Endringshistorikk

- **2026-06-12** Første gang hentet
- **2026-06-19** Innhold endret (se git-historikk for diff)
