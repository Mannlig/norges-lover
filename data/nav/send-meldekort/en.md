<!-- innholds-hash: 59af0377219816f5f4840fa92b6342fb7ca2e86695fe0867646645aec2b20ed4 -->

# Send employment status form - nav.no

## Kildeinformasjon

- **Kilde:** NAV (Arbeids- og velferdsdirektoratet) – https://www.nav.no/send-meldekort/en
- **Sist hentet:** 2026-06-27T07:40:09Z

## Innhold

How to


## Send employment status form

To be registered as a job seeker or to receive some benefits, you must submit an employment status form every 14 days.


### Contents on this page


### Contents on this page

In the employment status form you provide us with information about your work, activities, and absences in the last 14 days.

How you fill in the employment status form depends on what kind of benefit or follow-up you are receiving.

See what applies to you:

You may be asked to register as a job seeker and submit an employment status form to qualify for other follow-up or financial support.


#### Please confirm that you want to stay registered as a jobseeker

Everyone registered as a jobseeker with Nav must confirm that they still want to stay registered every 14 days.

You can find more information about how to submit the confirmation here.

If you are not sure how or why you should fill in an employment status form in your situation, we recommend that you

You fill in and submit the employment status form online.

Don’t forget to wait for confirmation that we have received your form before you close your browser.

- when your next employment status form is due
- which 14-day period the employment status form covers
The final deadline for submitting the employment status form is 23:00 Monday one week after the form’s 14-day period is over.


#### Send paper form by post

If you cannot send the employment status form online, you can print it out, fill it in by hand and send it by post.

- Go to the
- Enter your Norwegian national identification number.
- Download and print out the cover sheet and the form.
- Fill in and sign the form. Remember to write which 2 weeks the form is for.
- Send both the cover sheet and the form to us by post. Use the address on the cover sheet.
If you submit the form on paper, you need to keep track of the deadline yourself. Since we receive the form later and have to process it manually, you may have to wait longer for your payment.

Updated 08/20/2025


#### Chat with us

You will first meet chatbot Frida. On weekdays from 9 to 15, you can ask Frida to chat with an advisor.

Always open

Closed now, opens tomorrow at 9am


#### Write to us

Ask us a question and get a written answer

Response time is 3 business days. If you need answers faster, please see our chat service.


#### Call us at 55 55 33 33

Closed now, opens tomorrow at 9am


---
*Automatisk hentet fra [NAV](https://www.nav.no/send-meldekort/en) av norges-lover-bot.*

## Endringshistorikk

- **2026-05-16** Første gang hentet
- **2026-06-11** Innhold endret (se git-historikk for diff)
- **2026-06-19** Innhold endret (se git-historikk for diff)
- **2026-06-27** Innhold endret (se git-historikk for diff)
